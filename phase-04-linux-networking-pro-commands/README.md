...
In this phase, I learned and practiced important Linux networking commands that are used to check connectivity, troubleshoot DNS, inspect network interfaces, test ports, check services, verify routing paths, scan networks, download files, monitor live output, and manage firewall rules.

These commands are very important for DevOps engineers because most DevOps work involves servers, cloud instances, containers, applications, databases, CI/CD pipelines, APIs, and web services. All of these depend on networking.

---

## Learning Source

This learning is based on:

**Linux For DevOps In One Shot | Complete Beginners to Advanced Linux Hindi**

I am learning Linux step by step and documenting my notes, commands, examples, errors, fixes, and DevOps use cases in public.
...
- Which IP address is assigned to the server
- Which network interface is active
- Which route or gateway is being used
- Which ports are open
- Which services are listening
- Whether a firewall is allowing or blocking traffic
- Whether a website or API is responding
- Whether packets are being lost
- Where network delay is happening

Without Linux networking knowledge, it is difficult to troubleshoot servers, websites, containers, cloud machines, CI/CD runners, databases, APIs, and production applications.
...
These commands help check whether a server, website, or IP address is reachable.
...
| `mtr` | Combines ping and traceroute for live network analysis |

---

## 2. Network Interface and IP Commands

These commands help check IP addresses, network cards, routes, and interface status.

| Command | Purpose |
|---|---|
| `ip` | Modern command to check IP, routes, links, and ARP entries |
| `ifconfig` | Old command to check network interface information |
| `hostname` | Show or change system hostname |
...
These commands help check which services are running and which ports are open.

| Command | Purpose |
|---|---|
| `ss` | Show socket statistics and listening ports |
| `netstat` | Old command to show network statistics and listening ports |
| `nc` / `netcat` | Test TCP/UDP port connectivity |
| `telnet` | Old command used for simple port testing |

---

## 4. DNS Troubleshooting Commands

DNS means **Domain Name System**. These commands help check domain name resolution.

| Command | Purpose |
|---|---|
| `nslookup` | Basic DNS lookup command |
| `dig` | Advanced DNS lookup command |

---

## 5. Local Network Commands

These commands help check local network information.

| Command | Purpose |
|---|---|
| `arp` | Show IP-to-MAC address mapping |
| `ifplugstatus` | Check whether network cable/link is connected |
...
| Command | Purpose |
|---|---|
| `route` | View or manage routing table |
| `nmap` | Scan hosts, ports, and services |
...
| `curl` | Test APIs, HTTP requests, headers, and download data |
...
| Command | Purpose |
|---|---|
| `watch` | Run a command repeatedly and show live output |
...
| `-i 2` | Wait 2 seconds between packets |
| `-W 3` | Wait 3 seconds for reply |
...
`traceroute` is used when a website or server is slow or unreachable. It helps identify where the network problem is happening.

---

## 3. tracepath

The `tracepath` command is similar to `traceroute`, but it is simpler.
...
`tracepath` is useful for simple route checking. It can also help detect MTU issues.
...
| `tracepath` | Simple route checking | Easier and usually no root needed |
...
`mtr` is very useful for checking:
...
On many cloud servers, Wi-Fi interfaces are not available, so `iwconfig` may not show useful output.
